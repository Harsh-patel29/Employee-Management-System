...
const updateTaskStatus = AsyncHandler(async (req,res)=>{
  ...
  if(status === "Due_Today"){
    const task =await Task.findByIdAndUpdate(taskId,{Status:"In_Progress",EndDate:new Date().toLocaleDateString("en-CA").split("/").join("-")},{new:true})
    if(task.StartDate===undefined){
      await Task.findByIdAndUpdate(taskId,{StartDate:new Date().toLocaleDateString("en-CA").split("/").join("-")},{new:true})
    }
    ...
    const task =await Task.findByIdAndUpdate(taskId,{Status:"In_Progress",EndDate:prevDate.toLocaleDateString("en-CA").split("/").join("-"),StartDate:prevDate.toLocaleDateString("en-CA").split("/").join("-")},{new:true})
    return res.status(200).json(new ApiResponse(200,task,"Task status updated successfully"))
  }else{
...
const getTaskById = AsyncHandler(async (req,res)=>{
  ...
  return res.status(200).json(new ApiResponse(200,data,"Task fetched successfully"))
})

const uploadAttachmentImage = AsyncHandler(async(req,res)=>{
  const attachment = req.attachmentdetail;
  return res.status(200).json(new ApiResponse(200,attachment,"Task attachment uploaded successfully"))
})

const Attachment = AsyncHandler(async(req,res)=>{
  const attachment = req.attachment;
  return res.status(200).json(new ApiResponse(200,attachment,"Attachment uploaded successfully"))
})
...
const deleteUploadedImage = AsyncHandler(async(req,res)=>{
  ...
  try {
    ...
    throw new ApiError(500,error,"Something went wrong while deleting attachment")
  }
})
...
const deleteAttachment = AsyncHandler(async(req,res)=>{
  ...
  const attachment = await Task.findOne({Attachments:{$elemMatch:{public_id}}})

  if(!attachment){
    throw new ApiError(404,"Attachment not found")
  }
  try {
    const deletedAttachment = await Task.findOneAndUpdate({CODE:attachment.CODE},{$pull:{Attachments:{public_id}}},{new:true})
    await deleteFromCloudinary(public_id)
    return res.status(200).json(new ApiResponse(200,deletedAttachment,"Attachment deleted successfully"))
  } catch (error) {
    throw new ApiError(500,error,"Something went wrong while deleting attachment")
  }
})
...
const removeTodo = AsyncHandler(async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json(new ApiResponse(200, task, "Todo removed successfully"));
  } catch (error) {
...
export { createTask, getAlltasks, updateTaskStatus, updateTask, deleteTask, getTaskById, uploadAttachmentImage, Attachment, deleteAttachment, deleteUploadedImage, removeTodo };

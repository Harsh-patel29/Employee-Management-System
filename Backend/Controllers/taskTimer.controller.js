...
const createTaskTimer = AsyncHandler(async(req,res)=>{
    ...
    return res.status(200).json(new ApiResponse(200,taskTimer,"Task timer created successfully"))
})
...
const updateTaskTimer = AsyncHandler(async(req,res)=>{
    ...
    await taskTimer.save()
    return res.status(200).json(new ApiResponse(200,taskTimer,"Task timer updated successfully"))
})
...
const getTaskTimer = AsyncHandler(async(req,res)=>{
    ...
    const finalData ={
        ...
        User:taskTimer.User,
        StartTime:new Date(taskTimer.StartTime).toLocaleString("en-IN",{timeZone:"Asia/Kolkata"}),
        EndTime:taskTimer.EndTime ? new Date(taskTimer.EndTime).toLocaleString("en-IN",{timeZone:"Asia/Kolkata"}):null,
        Duration:taskTimer.Duration ? formatDuration(Number(taskTimer.Duration)) : null,
        Message:taskTimer.Message,
        createdAt:taskTimer.createdAt,
        updatedAt:taskTimer.updatedAt
    }
    return res.status(200).json(new ApiResponse(200,finalData,"Task timer fetched successfully"))
})
    ...
    const getAllTaskTimer = AsyncHandler(async(req,res)=>{
        ...
        TaskId:item.TaskId,
        User:item.User,
        StartTime:new Date(item.StartTime).toLocaleString("en-IN",{timeZone:"Asia/Kolkata"}),
        EndTime:item.EndTime ? new Date(item.EndTime).toLocaleString("en-IN",{timeZone:"Asia/Kolkata"}):null,
        ...
        return res.status(200).json(new ApiResponse(200,finalData,"All task timer fetched successfully"))
    })
    ...
    const deleteTaskTimer = AsyncHandler(async(req,res)=>{
        ...
        return res.status(200).json(new ApiResponse(200,taskTimer,"Task timer deleted successfully"))
    })

export {createTaskTimer,updateTaskTimer,getTaskTimer,getTaskByUser,getAllTaskTimer,deleteTaskTimer}
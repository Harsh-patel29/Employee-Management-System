...
const KanbanView = ({viewType = 'kanban'})=>{
    ...
    useEffect(()=>{
        const groupedTasks = {
            Backlog:filteredData?.filter(task=>task.Status === 'Backlog' && task.EndDate !== new Date().toLocaleDateString("en-CA").split("/").join("-") && !['Completed', 'Done', 'Deployed'].includes(task.Status)),
            In_Progress:filteredData?.filter(task=>task.Status === 'In_Progress' && task.EndDate !== new Date().toLocaleDateString("en-CA").split("/").join("-") && task.EndDate !== prevDate.toLocaleDateString("en-CA").split("/").join("-") && !['Completed', 'Done', 'Deployed'].includes(task.Status)),
            Due_Today:filteredData?.filter(task=>task.EndDate === new Date().toLocaleDateString("en-CA").split("/").join("-") && !['Completed', 'Done', 'Deployed'].includes(task.Status)),
            OverDue:filteredData?.filter(task=>task.EndDate === prevDate.toLocaleDateString("en-CA").split("/").join("-")&&!['Completed', 'Done', 'Deployed'].includes(task.Status)),
            Completed:filteredData?.filter(task=>['Completed', 'Done', 'Deployed'].includes(task.Status))
        }
        const boardViewTasks = {
            Backlog:filteredData?.filter(task=>task.Status === 'Backlog' && task.EndDate !== new Date().toLocaleDateString("en-CA").split("/").join("-") && !['Completed', 'Done', 'Deployed'].includes(task.Status)),
            In_Progress:filteredData?.filter(task=>task.Status === 'In_Progress' && task.EndDate !== new Date().toLocaleDateString("en-CA").split("/").join("-") && !['Completed', 'Done', 'Deployed'].includes(task.Status)),
            Done:filteredData?.filter(task=>task.Status === 'Done' && task.EndDate !== new Date().toLocaleDateString("en-CA").split("/").join("-")),
            Completed:filteredData?.filter(task=>['Completed'].includes(task.Status)),
            Deployed:filteredData?.filter(task=>task.Status === 'Deployed' && task.EndDate !== new Date().toLocaleDateString("en-CA").split("/").join("-")),
        }
        if(viewType === 'kanban'){
        ...
        <DragDropContext  onDragEnd={onDragEnd}>
            <div className="flex flex-row justify-evenly p-4">
                {Object.entries(columns).map(([columnId, columnTasks])=>(
                    <Droppable  direction="vertical" droppableId={columnId} key={columnId} isDropDisabled={false} isCombineEnabled={false} ignoreContainerClipping={false}>
                        {(provided,snapshot)=>(
                            <div className="w-full overflow-y-auto">
                             <div ref={provided.innerRef} {...provided.droppableProps} className={`bg-gray-100 h-[500px] shadow-xl w-64 rounded ${snapshot.isDraggingOver ? 'bg-gray-200' : ''}`}>

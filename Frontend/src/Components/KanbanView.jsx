...
const KanbanView = ({viewType = 'kanban'})=>{
    ...
    useEffect(()=>{
        const groupedTasks = {
            Backlog:task.filter(task=>task.Status === 'Backlog' && task.EndDate !== new Date().toLocaleDateString("en-CA").split("/").join("-") && !['Completed', 'Done', 'Deployed'].includes(task.Status)),
            In_Progress:task.filter(task=>task.Status === 'In_Progress' && task.EndDate !== new Date().toLocaleDateString("en-CA").split("/").join("-") && task.EndDate !== prevDate.toLocaleDateString("en-CA").split("/").join("-") && !['Completed', 'Done', 'Deployed'].includes(task.Status)),
            Due_Today:task.filter(task=>task.EndDate === new Date().toLocaleDateString("en-CA").split("/").join("-") && !['Completed', 'Done', 'Deployed'].includes(task.Status)),
            OverDue:task.filter(task=>task.EndDate === prevDate.toLocaleDateString("en-CA").split("/").join("-")&&!['Completed', 'Done', 'Deployed'].includes(task.Status)),
            Completed:task.filter(task=>['Completed', 'Done', 'Deployed'].includes(task.Status))
        }
        const boardViewTasks = {
            Backlog:task.filter(task=>task.Status === 'Backlog' && task.EndDate !== new Date().toLocaleDateString("en-CA").split("/").join("-") && !['Completed', 'Done', 'Deployed'].includes(task.Status)),
            In_Progress:task.filter(task=>task.Status === 'In_Progress' && task.EndDate !== new Date().toLocaleDateString("en-CA").split("/").join("-") && !['Completed', 'Done', 'Deployed'].includes(task.Status)),
            Done:task.filter(task=>task.Status === 'Deployed' && task.EndDate !== new Date().toLocaleDateString("en-CA").split("/").join("-") && !['Completed', 'Done', 'Deployed'].includes(task.Status)),
            Completed:task.filter(task=>['Completed', 'Done', 'Deployed'].includes(task.Status)),
            Deployed:task.filter(task=>task.Status === 'Deployed' && task.EndDate !== new Date().toLocaleDateString("en-CA").split("/").join("-") && !['Completed', 'Done', 'Deployed'].includes(task.Status)),
        }
        if(viewType === 'kanban'){
        ...
        <DragDropContext onDragEnd={onDragEnd}>
            <div className="flex flex-row justify-evenly p-4">
                {Object.entries(columns).map(([columnId, columnTasks])=>(
                    <Droppable  direction="vertical" droppableId={columnId} key={columnId} >
                        {(provided,snapshot)=>(

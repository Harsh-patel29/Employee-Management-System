  ...
  const taskCode = selectedTask?.split("-")[0];
    ...
    return (
      ...
      <div className=" rounded-2xl shadow-none p-6 w-[90%] max-w-md relative">
        <h2 className="text-xl font-semibold mb-6 text-gray-700">Time Tracker</h2>
        <div className="flex justify-between gap-4">
          {[{ label: "Hours", value: hours }, { label: "Minutes", value: minutes }, { label: "Seconds", value: seconds }].map((t, i) => (
            <div key={i} className="bg-gray-600 text-white rounded-md flex flex-col items-center justify-center w-24 h-24">
              <span className="text-2xl font-bold">{formatTime(t.value)}</span>
              <span className="text-xs mt-1 text-gray-200">{t.label}</span>
            </div>
          ))}
        </div>

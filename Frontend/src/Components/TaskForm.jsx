...
import { updateTask ,getTaskById,uploadAttachment,Attachment,resetUploadedImage,resetTask,deleteUploadedImage,deleteTodo} from "../feature/taskfetch/taskfetchSlice";
...
export default function TaskUpdateForm({ onSubmit, mode }) {
  ...
  const [openTaskAttachmentDialog, setOpenTaskAttachmentDialog] = useState(false)
  const [currentPublicId, setCurrentPublicId] = useState("")
  const[currentvalue,setCurrentvalue] = useState("")
  const users = useSelector((state) => state.getuser);
  const { user } = useSelector((state) => state.auth);
  const { tasks, createtask ,updatedTask,getTaskid,uploadedAttachment,uploadedImage,deletedAttachment,deletedTodo} = useSelector((state) => state.task);
  ...
  useEffect(() => {
    if (uploadedImage?.message) {
      ...
      const existingAttachments = Array.isArray(Tasks.Attachments) ? Tasks.Attachments : [];
...
const deleteUser = (userToDelete)=>{
  ...
  return (
    <>
      ...
      <Form {...control} >
        <form className="max-h-full min-h-screen" onSubmit={handleSubmit(onSubmit)}>
          <FormField 
            control={control}
            name="title"
            render={({ field }) => (
              ...
              >
                <FormLabel className="flex items-center">
                  ...
                  <button type="submit"
                    ...
                    onClick={()=>{handleUpdateTask("title", field.value)}}
                  >
                    <svg
                      class="w-6 h-6"
                      width="24"
                ...
                <div className="ml-5 text-[rgb(115,115,115)]">
                  #{taskid}
                </div>
              </FormItem>
            )}
                ...
                <FormItem className="w-full">
                  <FormLabel className="flex items-center justify-between text-[20px] font-[Inter,sans-serif] ml-5 font-[100] ">
                    ...
                    <div
                     ...
                     onClick={()=>{handleUpdateTask("description", field.value)} }
              ...
              render={({ field }) => (
                <FormItem className="w-full flex flex-col mt-6">
                  ...
                  <div className="flex w-full">
                    {showTodoField && (
                      ...
                      <button type="submit" 
                      onClick={() => {handleUpdateTask("todo", {todoTitle: field.value})
                        field.onChange("")
                      }}                      
                      className={`${
                        showTodoField
                      ...
                      {Tasks.todo  && (
                        <div className="flex overflow-y-auto flex-col ml-2 mt-2 w-full max-h-[300px]">
                          {Object.entries(Tasks.todo).map(([key, value]) => (
                            <div key={key} className="mb-1  flex justify-between items-center relative">
                              <div className={`flex  min-h-[45px] justify-start items-center pl-2 w-full h-full cursor-pointer rounded-md ${value.todoStatus ? ' bg-[rgb(211,248,211)]' : ' bg-[rgb(241,236,236)] hover:bg-[rgba(225,232,237,255)]'}`}>
                                <input 
                                  type="checkbox" 
                                ...
                                />
                              <span className={`flex relative h-full items-center pl-2 pb-1 text-[rgb(115,115,115)] ${value.todoStatus ? 'line-through ' : ''}`}>{value.todoTitle}</span> 
                              </div>
                      ...
                      {/* Update Todo Alert */}
                <AlertDialog open={openTodoUpdateDialog} onOpenChange={setOpenTodoUpdateDialog}>
                <AlertDialogContent>
                  <AlertDialogHeader>
                  ...
                  <AlertDialogFooter>
                    ...
                    onClick={() => {
                    dispatch(updateTask({data:todochecked, id: taskid}));
                ...
                <AlertDialogContent>
                  ...
                  <AlertDialogFooter>
                    ...
                    onClick={() => {
                      dispatch(deleteTodo({data:todoId,id:taskid}))
                      setOpenTodoDialog(false);
                    }}>
                      Delete
                    </AlertDialogAction>
...
text-decoration-line: underline decoration-[rgb(205,179,162)]"
                    ...
                    <div className="flex flex-wrap gap-2 p-2 ml-4 border-b-2 mr-4 border-gray-300">
                      {currentAttachments.map((value, index) => (
                        <div key={index} className="relative">
                          ...
                          <svg class="w-4 h-4 text-red-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24" style={{
                         transition: "transform 0.3s",
                         transform: "rotate(45deg)",
                        }}>
  <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 12h14m-7 7V5"/>
                          ...
                          <img src={value?.url} alt="Attachment" className="w-[90px] h-[90px] object-cover" />
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <AlertDialogContent>
                  ...
                  <AlertDialogFooter>
                    ...
                    onClick={() => {
                      ...
                      setCurrentAttachments(currentAttachments.filter(attachment => attachment.public_id !== currentvalue));
                      setOpenDialog(false);
                  ...
                  <div className="relative flex items-center w-full px-2 mt-2">
                    ...
                    <div className="absolute right-6 flex gap-3 mt-3">
                      <label htmlFor="attachment-button" className="cursor-pointer inline-block">
                        <svg
                          stroke=""
                          fill="rgb(51,141,181)"
                          stroke-width="0"
                      ...
                      <input type="file" id="attachment-button" multiple className="hidden" accept="image/*" 
                      onChange={(e)=>{
                      ...
                      <button id="comment-send-button" className="mb-3" onClick={()=>{
                    ...
                    <div key={value._id} className="h-auto">
                      <div className="flex items-center gap-2">
                        <p className="ml-2 bg-[rgba(61,66,179,0.92)] text-white rounded-full p-2 w-8 h-8 flex items-center justify-center">
                          {value.user ? value.user.split(" ").map(name=>name[0]).join("").toUpperCase() : user.user.Name.split(" ").map(name=>name[0]).join("").toUpperCase()}
                        </p>
                        ...
                        <p className="text-[rgb(115,115,115)]">{value.createdAt ? format(new Date(value.createdAt), "MMM, dd, yyyy, hh:mm:ss a") : ""}</p>
                        </div>
                      <p className="text-[rgb(115,115,115)] ml-2">{value.comment}</p>
                      <div className={`${value.Attachments.map((file)=>file.url===""?"w-0 h-0":"")}`}>
                        </div>
                        <div className="flex gap-2">
                          {value.Attachments.map((file, index)=>(
                            <div className="ml-2">
                            <img key={index} src={file.url} className="w-[90px] h-[90px] cursor-pointer" alt="Attachment" onClick={()=>{
...
{openImageDialog && selectedImage && (
  <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/70 backdrop-blur-sm" onClick={()=>{
      setOpenImageDialog(false)
    }}>
    <div className="relative bg-gradient-to-br bg-white  p-6 rounded-xl shadow-2xl max-w-[90%] max-h-[95%] border border-slate-700" onClick={(e)=>{
      e.stopPropagation()
    }}> 
      ...
      <img src={selectedImage} alt="Full View" className="flex max-w-full max-h-[80vh] rounded-lg shadow-lg" />
    </div>
  </div>
)}
                            ...
                            <DatePicker
                              ...
                              onChange={(date) => {
                                if (date) {
                                  ...
                                  handleUpdateTask("StartDate",localDate.toISOString().split("T")[0])
                                }
                              }}
                              inline
                            ...
                            <DatePicker
                              ...
                              onChange={(date) => {
                                if (date) {
                                  ...
                                  handleUpdateTask("EndDate",localDate.toISOString().split("T")[0])
                                }
                              }}
                ...
                render={({ field }) => (
                  <FormItem className="w-full">
                    <FormLabel className="flex h-11 rounded-md items-center gap-6 text-[20px] font-[Inter,sans-serif] ml-5 font-[100] bg-[rgba(231,235,245,0.66)] ">
                      ...
                      {Tasks?.EstimatedTime&&showEstimatedTimeField===false&&(
                        <div>
                          {Tasks?.EstimatedTime}
                        </div>
                      )}
                      <button
                        className={showEstimatedTimeField ? "cursor-pointer" : "hidden"} onClick={()=>{
                    ...
                    <FormLabel className="flex h-11 rounded-md items-center gap-6 text-[20px] font-[Inter,sans-serif] ml-5 font-[100] bg-[rgba(231,235,245,0.66)] ">
                      ...
                      <FormControl>
                       ...
                       <div className=" flex justify-end text-[15px] font-[Inter,sans-serif] mr-5 font-[100] bg-transparent">
                        {
                          Tasks?.Users?.length>0 && showUserDropDown===false&&(
                            <button className="cursor-pointer" onClick={()=>{setShowUserDropDown(true)}}>
...
<path d="M16.59 8.59L12 13.17 7.41 8.59 6 10l6 6 6-6z"></path>
                        ...
                        {Tasks?.Users?.length>0 && showUserDropDown===true&&(
                        <button className="cursor-pointer" onClick={()=>{setShowUserDropDown(false)}}>
<svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 24 24" class="tasks-item-icon" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
<path fill="none" d="M0 0h24v24H0z"></path>
<path d="M12 8l-6 6 1.41 1.41L12 10.83l4.59 4.58L18 14z"></path>
                       ...
                       {
                    showUserDropDown===true && Tasks?.Users &&Tasks?.Users?.length>0 &&(
                      ...
                      <div className="flex flex-col justify-end font-light text-[15px] font-[rgb(115,115,115)]">
                              {
                                Tasks?.Users?.map((user,index)=>(
                                  <div key={index} className="cursor-pointer flex justify-between items-center h-auto pt-1 pb-1 pr-2">
                                      <span className="flex gap-4 items-center">
                                        <p  className="ml-1 bg-[rgba(61,66,179,0.92)] text-white rounded-full w-8 h-8 flex items-center justify-center">
        {user.split(" ").map(name => name[0]).join("").toUpperCase()}
                                        </p>
                                        <h6>
        {user}
        </h6>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="w-full">
                    <FormLabel className="flex h-11 rounded-md items-center gap-6 text-[20px] font-[Inter,sans-serif] ml-5 font-[100] bg-[rgba(231,235,245,0.66)] ">
                      <div className="ml-2.5 text-[rgb(155,159,167)]">
                       <svg stroke="currentColor" fill="currentColor" stroke-width="0" version="1.1" viewBox="0 0 16 16" class="tasks-item-icon" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
                      ...
                      <label className="cursor-pointer ">
                       ...
                       <input type="file" id="attachment-button" multiple className="hidden" accept="image/*" onChange={(e) => handleAttachmentChange(e)} /> 
                      </label>
                          </div>
                      <div className="flex w-full justify-end text-[15px] font-[Inter,sans-serif] mr-5 font-[100] bg-transparent">
                        {
                         Tasks?.Attachments?.length>0 && showdropDown===false&&(
                        <button className="cursor-pointer" onClick={()=>{setShowdropDown(true)}}>
                        <svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 24 24" class="tasks-item-icon" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
                        <path fill="none" d="M0 0h24v24H0z"></path>
<path d="M16.59 8.59L12 13.17 7.41 8.59 6 10l6 6 6-6z"></path>
                        </svg>
                        ...
                        {Tasks?.Attachments?.length>0 && showdropDown===true&&(
                        <button className="cursor-pointer" onClick={()=>{setShowdropDown(false)}}>
<svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 24 24" class="tasks-item-icon" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
<path fill="none" d="M0 0h24v24H0z"></path>
<path d="M12 8l-6 6 1.41 1.41L12 10.83l4.59 4.58L18 14z"></path>
</svg>
...
setOpenImageDialog(true)
        ...
        {file.orignalname}.{file.format}
                ...
                />
              <AlertDialog open={openTaskAttachmentDialog} onOpenChange={setOpenTaskAttachmentDialog}>
                <AlertDialogContent>
                  <AlertDialogHeader>
                    <AlertDialogTitle>Delete Attachment</AlertDialogTitle>
                    <AlertDialogDescription>
                    ...
                    onClick={() => {
                       ...
                       if (Tasks?.Attachments) {
                         const updatedAttachments = Tasks.Attachments.filter(
                           attachment => attachment.public_id !== currentPublicId
                         );
                         handleUpdateTask("Attachments", updatedAttachments);
                       }
                    ...
                    <FormLabel className="flex h-11 rounded-md items-center gap-6 text-[20px] font-[Inter,sans-serif] ml-5 font-[100] bg-[rgba(231,235,245,0.66)] ">
                      ...
                      <FormControl>
                        <Input
                          ...
                          defaultValue={Tasks?.createdAt ? `${new Date(Tasks.createdAt).toLocaleDateString('en-GB')} at ${new Date(Tasks.createdAt).toLocaleTimeString()}` : ''}
                          readOnly

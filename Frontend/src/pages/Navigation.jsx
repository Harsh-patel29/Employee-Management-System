...
const Navigation = () => {
  ...
  const getActiveKeyFromPath = () => {
    for (let item of menuItems) {
      ...
      } else if (currentPath.startsWith(`/${item.key}`)) {
        return item.key;
      }
    }
    return 'Dashboard';
  };
  const [active, setActive] = useState(getActiveKeyFromPath);
    ...
    >
      ...
      >
        ...
        >
          {menuItems.map((item) => {
            const childLinks =
              item.children?.map((val) => `/${val.link}`) || [];
            const isActive = childLinks.some((link) =>
